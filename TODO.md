...
## v0.3 — targeted refactor (in progress)
...
- [x] WS4 — gate kill()'s pre-SIGTERM `lsof` sweep on port-declaring (and still-live) scripts
...
- Mobile feedback unification: the companion still uses native `window.confirm`/`alert` (desktop is fully on glass toast/ConfirmDialog) — add a lightweight mobile toast/confirm sheet.
