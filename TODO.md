...
- [x] Rust formatting maintenance — done: all 228 pre-existing positional `format!`/logging arguments inlined (machine-applied `clippy --fix`, tests green) and the temporary `uninlined_format_args` allowance removed, so every Clippy warning is now fatal in CI with no exemptions.
